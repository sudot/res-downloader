  ...
  return new Promise(async (resolve: any, reject) => {
    try {
      ...
      proxy.intercept(
        ...
        (req, res) => {
          ...
          try {
            ...
            if (
              !media?.decodeKey ||
              global.videoList.hasOwnProperty(url_sign) === true
            ) {
              return
            }
            ...
            log.log('proxy.intercept request', e.toString())
          }
        }
      )
      ...
      proxy.intercept(
        ...
        async (req, res) => {
          if (
            req.url.includes('/web/pages/feed') ||
            req.url.includes('/web/pages/home')
          ) {
      ...
      proxy.intercept(
        ...
        async (req, res) => {
          ...
          if (
            req.url.includes(
              'web/web-finder/res/js/virtual_svg-icons-register.publish'
            )
          ) {
      ...
      proxy.intercept(
        ...
        async (req, res) => {
          try {
                ...
                if (global.videoList.hasOwnProperty(url_sign) === false) {
                  ...
                  win.webContents.send(
                    ...
                    Object.assign({}, resObject, {
                      ...
                      platform: urlInfo.hostname,
                      size: toSize(
                        res?._data?.headers?.['content-length'] ?? 0
                      ),
                      type: ctype,
                      type_str: 'video',
                    })
              ...
              case 'image/vnd.adobe.photoshop':
                win.webContents.send(
                  ...
                  Object.assign({}, resObject, {
                    ...
                    url_sign: url_sign,
                    platform: urlInfo.hostname,
                    size: res?._data?.headers?.['content-length']
                      ? toSize(res?._data?.headers?.['content-length'])
                      : 0,
                    type: ctype,
                    type_str: 'image',
                  })
            ...
            log.log('proxy.intercept response', e.toString())
          }
        }
      )
    } catch (e) {
